...
create table director(
 ...
 create table movie_direction(
	...
	constraint director_fk foreign key(dir_id) references director (dir_id),
	constraint movie foreign key (mov_id)  references movie(mov_id));
...
create table reviewer(
	...
	constraint reviewer primary key (rev_id));
        
 create table genres(
	gen_id integer not null,
	gen_title char(20) not null,
	constraint genres primary key(gen_id));
...
create table rating(
	...
	constraint reviewer foreign key(rev_id) references reviewer(rev_id));
